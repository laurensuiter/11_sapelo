...
weather <- read_csv(here("data", "weather_monthsum.csv")) %>% 
  # separate site to extract the state abbrevations only
  separate(site, into = c("city", "state"), sep = ",") %>%
  # relocate the separated or newly created colnames 
  relocate(c("city", "state"), .before = "year") %>%
...
weather_recipe <- recipe(strength_gtex ~ ., data = weather_train) %>% # Remove identifier columns and months not in growing season
  ...
  update_role(c(year, state), new_role = "ID")
...
publication_ready <- final_fit %>%
  ...
  geom_point(aes(fill = strength_gtex), shape = 21, alpha = 0.7, show.legend = F) + 
...
```{r}
knitr::purl("04-07_sapelo_partial.qmd", output = "sapelo_script.R", documentation = 0)
```
